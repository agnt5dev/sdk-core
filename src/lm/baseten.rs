...
const MODEL_PREFIX: &str = "baseten";
...
impl BasetenConfig {
    ...
    pub fn from_env() -> SdkResult<Self> {
        let api_key = env::var("BASETEN_API_KEY").map_err(|_| SdkError::Configuration {
            message: "BASETEN_API_KEY must be set".to_string(),
            field: Some("BASETEN_API_KEY".to_string()),
        })?;

        let mut config = BasetenConfig::new(api_key);

        if let Ok(base) = env::var("BASETEN_BASE_URL") {
...
impl BasetenProvider {
    pub fn new(config: BasetenConfig) -> SdkResult<Self> {
        let inner_config = OpenAiChatConfig::new(config.api_key)
            .with_base_url(config.base_url)
            .with_auth_scheme(config.auth_scheme)
...
#[async_trait]
impl LanguageModel for BasetenProvider {
