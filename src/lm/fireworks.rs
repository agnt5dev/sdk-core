...
const MODEL_PREFIX: &str = "fireworks";
...
impl FireworksConfig {
    ...
    pub fn from_env() -> SdkResult<Self> {
        let api_key = env::var("FIREWORKS_API_KEY").map_err(|_| SdkError::Configuration {
            message: "FIREWORKS_API_KEY must be set".to_string(),
            field: Some("FIREWORKS_API_KEY".to_string()),
        })?;

        let mut config = FireworksConfig::new(api_key);

        if let Ok(base) = env::var("FIREWORKS_BASE_URL") {
...
impl FireworksProvider {
    pub fn new(config: FireworksConfig) -> SdkResult<Self> {
...
#[async_trait]
impl LanguageModel for FireworksProvider {
