...
const DEFAULT_BASE_URL: &str = "https://api.together.ai/v1";
const MODEL_PREFIX: &str = "together";
...
impl TogetherConfig {
    ...
    pub fn from_env() -> SdkResult<Self> {
        let api_key = env::var("TOGETHER_API_KEY").map_err(|_| SdkError::Configuration {
            message: "TOGETHER_API_KEY must be set".to_string(),
            field: Some("TOGETHER_API_KEY".to_string()),
        })?;

        let mut config = TogetherConfig::new(api_key);

        if let Ok(base) = env::var("TOGETHER_BASE_URL") {
...
impl TogetherProvider {
    pub fn new(config: TogetherConfig) -> SdkResult<Self> {
